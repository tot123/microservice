...
import com.atguigu.gateway.enums.ServerRouteEnum;
        ...
        .forEach(routeDefinition -> routeDefinition.getPredicates().stream()
            ...
            .forEach(predicateDefinition -> {
                  ...
                  if (StringUtils.isNotEmpty(swaggerInfo)) {
                    resources.add(swaggerResource(swaggerInfo, predicateDefinition.getArgs().get(
                        NameUtils.GENERATED_NAME_PREFIX + "0").replace("/**", API_URI)));
                  }
                }
            ));
    return resources;
